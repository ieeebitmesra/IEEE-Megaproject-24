...
const query = () => {
  ...
  const [responseKeysOne, setResponseKeysOne] = useState([]);
  const [responseKeysTwo, setResponseKeysTwo] = useState([]);

  const tokenList = [];

  const check = (token)=>{
    if(response == null) return;
    if(responseKeysTwo.includes(token[0])){
      for(let i=0;i<response.model_2_loc.length;i++){
        if(response.model_1_loc[i][1] == token[1]){
  ...
  useEffect(()=>{
    if(response){
      let keys = Object.keys(response.fuzzy_matches_1);
      setResponseKeysOne(keys);
      keys = Object.keys(response.fuzzy_matches_2);
      setResponseKeysTwo(keys);
    }
  },[response,dispatch])
                  ...
                  <div className="flex flex-col justify-normal items-start px-24 w-full h-full box-border rounded-[30px] pt-[30px] pb-5">
                    ...
                    <div>
                      {newQueryLoading ? (
                        ...
                        <>
                          {response && <div className="mt-4 border w-full p-2">Primary Model</div> }
                          {response && (
                          ...
                          {response &&
                            ...
                            {response && <div className="mt-4 border w-full p-2">Secondary Model</div> }
                            {response && (
                            <div className="grid grid-cols-3 py-4 text-xl border-b-2 border-Primary border-opacity-40">
                              <h3 className="">Token</h3>
                              <h3>Entity Type</h3>
